...
class TechDataBase:
    ...
    def createTable(self, table: str):
        ...
        if table == "user":
            ...
            self._cursor.execute("CREATE TABLE user_key (username VARCHAR(255) NOT NULL, user_key VARCHAR(255) NOT NULL, PRIMARY KEY( username ) USING BTREE, CONSTRAINT username FOREIGN KEY (username) REFERENCES cb.users (username) ON UPDATE NO ACTION ON DELETE NO ACTION)")
        elif table == "password_recovery":
            self._cursor.execute("CREATE TABLE password_recovery (user_id MEDIUMINT(6) UNSIGNED ZEROFILL NOT NULL, access_hash VARCHAR(100) NOT NULL, expires DATETIME NOT NULL, PRIMARY KEY (user_id) USING BTREE, CONSTRAINT user_id_key FOREIGN KEY (user_id) REFERENCES cb.users (id) ON UPDATE NO ACTION ON DELETE NO ACTION)")
        else:
