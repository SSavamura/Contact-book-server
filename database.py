...
class TechDataBase:
    ...
    def createTable(self, table: str):
        ...
        if table == "user":
            self._cursor.execute("CREATE TABLE user (user_id MEDIUMINT(6) UNSIGNED ZEROFILL NOT NULL AUTO_INCREMENT, username VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL, PRIMARY KEY( user_id ) USING BTREE, INDEX username ( username ) USING BTREE)")
        elif table == "user_key":
